...
在文献 \tmem{从零构建量子力学}（quantum.tex）中，我们从三条基本公理出发推导了量子力学的时间演化方程。推导的关键步骤是路径积分形式，其中我们用到了 $\mathbb{R}^d$ 上的傅里叶变换来得到 $k\dot{x}$ 项：
\[
...
\begin{enumerate}
  ...
  \item 流形上的对称性（一般协变性，即 $\tmop{Diff}(M)$ 不变性）强制截断阶 $N_{\tmop{cut}} = 2$，得到 Laplace--Beltrami 算子作为动能项。
\end{enumerate}
...
在时间切片方案中，我们从 $x_0$ 出发传播到 $x_1$。固定前一点 $x_0$，后一点 $x_1$ 在其法坐标邻域内。将 $\delta_{x_0}(x_1)$ 和 $r(x_1,x_0,t)$ 用 (\ref{eq:delta-manifold}) 和 (\ref{eq:r-inv}) 表示，并记 $\xi_0 = \exp_{x_0}^{-1}(x_1)$，$p_0 \in T_{x_0}^*M$，$J_{x_0}(\xi_0)$ 为 Jacobian：
...
即使 $r$ 在局部与 $\mathbb{R}^d$ 中的形式完全相同（例如 $r(x,y,t) \propto \nabla_x^2 \delta_y(x)$），$J_y(\xi)$ 和度规的存在也会在 $R_2$ 中引入曲率依赖项。经过计算，更一般地，在流形上，二阶矩必须与度规成比例（由后续的 $\tmop{Diff}(M)$ 不变性决定）：
\begin{equation}
...
\chapter{$\tmop{Diff}(M)$ 不变性与 $N_\text{cut}=2$}

在 $\mathbb{R}^d$ 上，Galilean 不变性选定了 $N_\text{cut}=2$。但 Galilean 不变性依赖于平坦时空的直译结构（惯性系、匀速直线运动），这些概念在弯曲流形上不再有全局意义。自然的替代是\textbf{一般协变性}（general covariance），即 $\tmop{Diff}(M)$ 不变性：物理定律在任意微分同胚下形式不变。
...
\begin{description}
  ...
  \item[$n=2$] $R_2^{\alpha\beta}(x)$ 是 $(0,2)$ 阶对称协变张量。在 $M$ 上，度规 $g^{\alpha\beta}$ 是唯一自然的 $(2,0)$ 阶张量，其逆 $g_{\alpha\beta}$ 是 $(0,2)$ 阶张量。$\tmop{Diff}(M)$ 不变性要求 $R_2^{\alpha\beta} \propto g^{\alpha\beta}$。比例系数由物理常数确定：
    \[
...
因此 $\tmop{Diff}(M)$ 不变性 + 最小耦合原理强制 $N_\text{cut} = 2$。
...
\begin{enumerate}
  ...
  \item \textbf{$\tmop{Diff}(M)$ 不变性}（一般协变性）取代 Galilean 不变性，强制 $N_\text{cut}=2$ 并固定 $R_2 = -g^{\alpha\beta}/m$，从而得到 Laplace--Beltrami 算子作为动能项。
\end{enumerate}
